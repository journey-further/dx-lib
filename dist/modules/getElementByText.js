 ...
 */
export const getElementByText = (tag, query, parentSelector) => {
    // Get the args first
    if (!tag)
        throw new Error("Provide a HTML element tag to search for as arg 1");
    if (!query)
        throw new Error("Provide a query string or regex pattern as arg 2");
    const elementWithText = Array.from(document.querySelectorAll(tag)).find((elem) => {
        if (typeof query === "string") {
            return (elem === null || elem === void 0 ? void 0 : elem.textContent) && elem.textContent === query;
        }
        return (elem === null || elem === void 0 ? void 0 : elem.textContent) && query.test(elem.textContent);
    });
