export const jfEvents = ["load", "error", "track"];
export const isJfEvent = (event) => jfEvents.includes(event);
/**
...
export const emitEvent = (type, experiment, err) => {
    ...
    if (typeof experiment === "undefined")
        throw new Error("Could not find global experiment information object");
    if (!(experiment === null || experiment === void 0 ? void 0 : experiment.variant))
        throw new Error("Variant is missing from global experiment information object");
    if (!/^(?:[A-Z]|CONTROL)$/.test(experiment.variant))
        throw new Error("Variant is invalid, it should be a single letter or 'CONTROL'");
    if (!(experiment === null || experiment === void 0 ? void 0 : experiment.ticketId))
        throw new Error("Ticket ID is missing from global experiment information object");
    ...
    if (type === "load") {
        window.dispatchEvent(new CustomEvent("jf-wx-test", {
            detail: {
                // eslint-disable-next-line no-undef
                ticket: experiment.ticketId,
                // eslint-disable-next-line no-undef
    ...
    if (type === "track") {
        window.dispatchEvent(new CustomEvent("jf-wx-track", {
            detail: {
                // eslint-disable-next-line no-undef
                ticket: experiment.ticketId,
                // eslint-disable-next-line no-undef
