 ...
 *
 * @param callback A function to execute when timeout expires
 * @param timeout A number to indicate how much time should pass before executing he callback
 * @param id A unique identifier for the timer
 * @returns An object with information about this timer
 */
export const useSetTimeout = (callback: () => void, timeout: number, id: string): JfTimerObject => {
  if (typeof callback !== "function") throw new Error("Arg 1 must be a function");
  ...
  if (!!currentTimeout) {
    // clear it
    clearTimeout(currentTimeout.timer);
